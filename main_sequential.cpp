...
int main(int argc, char *argv[]) {
	...
	vector<double> epsvector; //epsilon
	...
	prefix prefix(eng, filename, id_backup);//read data
	...
	for(int x = 0; x < epsvector.size(); x ++){
		...
		fstream xxf; xxf.open("freuqecy_distribution.txt", ios::app);//"frequency_distribution.txt" stores the frequency distribution
		xxf<<"epsilon= "<<epsilon<<" "<<endl;
		xxf.close();

		vector<double> temp; temp.resize(rep, 0);
		vector<vector<double>> lerr_fscore; 
		...
		for (int i = 0; i < rep; i++) {
			...
			std::random_shuffle ( id.begin(),id.end() );//shuffle data

			prefix.initialRoot(id);
		
			vector<vector<int>> topks;
			map<vector<int>, double> statistic =	getAccurateResult(kvector.back(), prefix.orgraw, topks);

			vector<pair<vector<int>, double>> result = prefix.PST3(epsilon, id, 0.8, kvector.back());//privTrie


			for(int ki = 0; ki < kvector.size(); ki++){
				lerr_fscore[ki][i] = topk_precision(result, topks, kvector[ki]);
